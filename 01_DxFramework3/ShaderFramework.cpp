...
D3DXVECTOR4 gWorldCameraPosition(0.f, 0.f, -200.f, 1.f);
...
void RenderScene()
{
	...
	D3DXVECTOR3 vEyePt(gWorldCameraPosition.x, gWorldCameraPosition.y, gWorldCameraPosition.z);	// 카메라의 현재 위치
	D3DXVECTOR3 vLookatPt(0.0f, 0.0f, 0.0f);		// 카메라가 바라보는 곳의 위치
	D3DXVECTOR3 vUpVec(0.0f, 1.0f, 0.0f);		// 카메라의 위쪽을 가리키는 벡터
	...
	// 프레임마다 0.4도씩 회전을 시킨다.
	gRotationY += 0.2f * PI / 180.f;
	if (gRotationY > 2 * PI)
	{
		gRotationY -= 2 * PI;
	}

	// 월드 행렬을 만든다. 월드의 원점에 물체를 놓아둔다고 가정하므로 단위행렬로 놔둠.
...
bool LoadAssets()
{
	...
	// 쉐이더 로딩
	gpLightingShader = LoadShader("Lighting2.fx");
	if (!gpLightingShader)
	{
		return false;
	}
